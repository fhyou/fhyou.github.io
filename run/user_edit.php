...
if(!$_REQUEST['id'] || !is_numeric($_REQUEST['id'])){
  err('<img src="images/i.gif" /> 文章参数出错！<br />问题分析：1、您可能未选择文章；2、参数传递出错');
}
...
$content.='<div style="text-align:right;color:#FF5500;">编辑时间：'.gmdate('Y/m/d H:i:s',time()+(floatval($web['time_pos'])*3600)).'</div>';
...
if($result=mysql_query("SELECT * FROM bbslistdata WHERE id='".$_REQUEST["id"]."'",$db)){ //结果集
  ...
  if(!confirm_manager()){
    if($row['author_ip']!=$cookie[0]){
      err('<img src="images/i.gif" /> 经查你与该文作者不符！');
    }
  }
  mysql_query("UPDATE bbslistdata SET title='".$subject."',`text`='".$content."',pic='".$pic."',fil='".$fil."',enc='".$enc."',area_id='".$_REQUEST['area_id']."' WHERE id='".$_REQUEST["id"]."'");
}

if(mysql_affected_rows()>0){
  mysql_close();
  alert('<img src="images/ok.gif" /> 编辑成功！','article.php?id='.$_REQUEST["id"].'');
}else{

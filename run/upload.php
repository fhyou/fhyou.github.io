...
if($pics=@glob('data/upload/'.urlencode($cookie[0]).'_'.$date.'_*')){
  ...
  //文件命名
  $upload_filename=urlencode($cookie[0]).'_'.$date.'_'.($nn+1);
}else{
  $upload_filename=urlencode($cookie[0]).'_'.$date.'_1';
}

$upload_filename=str_replace('.','',$upload_filename).'.'.strtolower($matches[1]);
...
if(move_uploaded_file($_FILES['uploadfile']['tmp_name'],$upload_dir.'/'.$upload_filename)) {
  /*if(preg_match('/<\?|\?>|eval|POST|base64_decode|base64_encode/i',@file_get_contents($upload_dir.'/'.$upload_filename))){
    @unlink($upload_dir.'/'.$upload_filename);
    echo '<script>alert("提示！禁止提交。该文件含有禁止的代码！");</script>';
    die();
  }*/

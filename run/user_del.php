...
if(confirm_login()==false)
  err('<img src="images/i.gif" /> 请<a href="user_login.php">登录</a>！');
...
if($_REQUEST['limit']=='refresh'){
  ...
  $eval='
  mysql_query(\'UPDATE bbslistdata SET topdate="\'.$nowdate.\'" WHERE id="\'.$each.\'" and author_ip="\'.$cookie[0].\'"\');
  ';
  ...
  if($each<=5){
    ...
    $step++;
    mysql_query(\'DELETE FROM bbslistdata WHERE id="\'.$each.\'" AND author_ip="\'.$cookie[0].\'"\');
  }
  ';
}elseif($_REQUEST['limit']=='top'){
  ...
  if($cookie[2]!='manager'){
    if(abs($row['topcount'])>0 && $row['topdate'] && $row['topdate']>$date){
      ...
      //err('<img src="images/ok.gif" /> 你可以置顶的数量为'.$row['topcount'].'条，置顶期限至'.get_date($row['topdate']).'');
    }else{
      err('<img src="images/i.gif" /> 你没有置顶权限或置顶权限期限已过！');
    }
  }
  $eval='
  mysql_query(\'UPDATE `bbslistdata` SET `topdate`="\'.$row[\'topdate\'].\'" WHERE `id`="\'.$each.\'" and `author_ip`="\'.$cookie[0].\'"\');
  ';
}elseif($_REQUEST['limit']=='ctop'){
  $result=mysql_query('SELECT topcount,topdate FROM bbsmember WHERE username="'.$cookie[0].'"',$db); //结果集
  $row=mysql_fetch_assoc($result);
  mysql_free_result($result);
  $date=gmdate('YmdHis',time()+(floatval($web['time_pos'])*3600));
  if($cookie[2]!='manager'){
    if(abs($row['topcount'])>0 && $row['topdate'] && $row['topdate']>$date){
      ...
      err('<img src="images/i.gif" /> 你没有置顶权限或置顶权限期限已过！所以不必执行“取消置顶”功能');
    }
  }
  $eval='
  mysql_query(\'UPDATE `bbslistdata` SET `topdate`="\'.$date.\'" WHERE `id`="\'.$each.\'" and `author_ip`="\'.$cookie[0].\'"\');
  ';
}else{
...
if($step>0 && $cookie[2]!='manager'){ //把分减掉
  ...
  mysql_query('UPDATE bbsmember SET point='.$now_ponit.' WHERE username="'.$cookie[0].'"',$db);
  echo '
...
if(mysql_affected_rows()>0){
  alert('<img src="images/ok.gif" /> 执行成功！','user_list.php');
}else{

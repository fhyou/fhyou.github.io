...
/*网站设置*/
 ...
 require('inc/function/write_file.php');
  ...
  if(!($_POST['manager']!='' && preg_match('/^[\x80-\xff\w]{3,45}$/',$_POST['manager']))){
    err('<img src="images/i.gif" /> 管理员起始名称不能空且请用汉字、数字、英文及下划线组成！长度范围为3-45字符！');
  }
  if($_POST['password']==''){
    err('<img src="images/i.gif" /> 管理员密码不能留空！');
  }
  if(preg_match('/[\s\r\n]+/',$_POST['password'])){
    err('<img src="images/i.gif" /> 管理员密码不能有空格！');
  }
  if(strlen($_POST['password'])>30 || strlen($_POST['password'])<3){
    err('<img src="images/i.gif" /> 管理员密码长度是3-30字符！');
  }
  ...
  if(preg_match('/[^a-zA-Z0-9\|]+/',$_POST['_17_type']))
    err('<img src="images/i.gif" /> 允许上传的影音文件只应用合理的后缀名及分隔符|填写！');
  if(preg_match('/[^a-zA-Z0-9\|]+/',$_POST['_18_type']))
    err('<img src="images/i.gif" /> 允许上传的其它文件只应用合理的后缀名及分隔符|填写！');
...
$web[\'sitename\']=\''.(($_POST['sitename']!='')?filter1($_POST['sitename']):'我的网站').'\';  //站点名称
$web[\'description\']=\''.filter1($_POST['description']).'\';  //站点描述
$web[\'keywords\']=\''.filter1($_POST['keywords']).'\';  //关键字
$web[\'manager\']=\''.$_POST['manager'].'\';  //基础管理员名称
$web[\'password\']=\''.$_POST['password'].'\';  //基础管理员密码，注：系统出现一切故障时以基础管理员名称和密码为准
$web[\'cssfile\']=\''.(file_exists('css/'.$_POST['cssfile'].'/style.css')?$_POST['cssfile']:'blue').'\';  //站点默认风格
...
$web[\'mess_wordcount\']='.(abs((int)$_POST['mess_wordcount'])>0?abs((int)$_POST['mess_wordcount']):400).';  //短信发表最多字符数
$web[\'mess_count\']='.(abs((int)$_POST['mess_count'])>0?abs((int)$_POST['mess_count']):100).';  //站内短信最多保存条数
...
$web[\'send_mx\']='.abs((int)$_POST['send_mx']).';  //1每日发表限量 0不限量
...
$web[\'pic_markwords\']=\''.filter1($_POST['pic_markwords']).'\';  //水印文字
$web[\'pic_markfont\']=\''.$_POST['pic_markfont'].'\';  //水印字体
  ...
  if($_POST['manager']!=$web['manager'] || $_POST['password']!=$web['password']){
    if($db=@mysql_connect($sql['host'],$sql['user'],$sql['pass'])){
      if(@mysql_select_db($sql['name'],$db)){
        ...
        @mysql_query('UPDATE bbsmember SET username="'.$_POST['manager'].'",password="'.$_POST['password'].'" WHERE username="'.$web['manager'].'"',$db);
      }
    ...
    if($sqlaccess==''){
      err('<img src="images/i.gif" /> <font color=#FF5500>由于基础管理员信息(如名称或密码)有变，但数据库(或数据表)连接不成功或被删除或尚未安装，拒绝更改设置！<br />请检查数据库及数据表</font>');
    }
  }
  /* 写 */
  write_file('inc/set.php',$text);
  alert('<img src="images/ok.gif" /> 执行成功！'.$sqlaccess,'admin_set.php');

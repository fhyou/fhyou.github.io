...
//栏目分类设置
    ...
    foreach($_POST['column_id'] as $fid_old=>$fid_new){
      ...
      if(count($_POST['class_id'][$fid_old])>0){
        if(count($_POST['class_id'][$fid_old])>count(array_unique($_POST['class_id'][$fid_old])))
          err('<img src="images/i.gif" /> 新分论坛'.$fid_new.'的版区不能有相同的ID序号！');
        if(count($_POST['class_name'][$fid_old])>count(array_filter(preg_replace('/^\s+|\s+$/','',$_POST['class_name'][$fid_old]))))
          err('<img src="images/i.gif" /> 新分论坛'.$fid_new.'的版区'.$cid_new.'名称不能有空值！');
        ...
        err('<img src="images/i.gif" /> 新分论坛'.$fid_new.'的版区不能没有栏目！');
	  }
	}
    @ksort($area_new);
  }
  ...
  if($web['area']==$area_new){
    err('<img src="images/i.gif" /> 您对版区设置未做任何更改！');
  }
  if(!$area_new){
...
?>';
  ...
  alert('<img src="images/ok.gif" /> 栏目分类设置成功！'.$errr,'admin_set_area.php');

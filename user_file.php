...
function postChk(theForm){
  var po=theForm.password.value;
  if(po!='' && po.match(/[\s\r\n]+/)){
    alert('原密码不能有空格！请重输');
	theForm.password.focus();
    return false;
  }
  if(po!='' && (po.length>30 || po.length<3)){
    alert('原密码长度是3-30字符！');
	theForm.password.focus();
    return false;
  }
  var pn=theForm.password_new.value;
  if(pn!='' && pn.match(/[\s\r\n]+/)){
    alert('新密码不能有空格！请重输');
	theForm.password_new.focus();
    return false;
  }
...
if(isset($yes)){
  ...
  if(!isset($err)){
?>
<div class="re_author">我的名片</div>

<a href="user_card.php?username=<?php echo urlencode($row['username']); ?>" target="_blank">预览我当前的名片</a><br />
        ...
        <form action="run.php?run=user_reg" method="post" enctype="multipart/form-data" name="regform" id="regform" onsubmit="return postChk(this)">
          ...
          <table cellspacing="0" border="0" cellpadding="0" width="100%">
            ...
            <tr>
              ...
              <td><input name="password_new" type="password" onkeydown="if(event.keyCode==32){alert('密码不能有空格');return false;}" style="width:200px" value="" maxlength="40" />
                  （不填则默认原密码）</td>
            </tr>
            <tr>
            ...
            <tr>
              <td width="100" align="right">就读学校：</td>

...
<div class="area">
  <div class="area_title"><img src="css/<?php echo $web['cssfile']; ?>/area_title.gif" align="absmiddle" /> <a href="forum.php">课后讨论首页</a> &gt; 我要发表帖子 &gt; 选择类目</div>
</div>
...
-->
  ...
  $youclass=user_class(abs($cookie[1]));
  echo '欢迎：'.$cookie[0].' '.$youclass.'<br />
  ...
  $imcode='<br /><br /><u><b><span style="color:#FF6600">*</span> 匿名发表请填验证码：</b></u><br /><table width="220" border="0" cellspacing="0" cellpadding="0"><tr><td><input name="imcode" type="text" style="width:100px" /></td><td><iframe src="js/imcode.html" id="imFrame" name="imFrame" frameborder="0" width="100" height="24" scrolling="No"></iframe></td></tr></table>';
}
?>
<?php
if($yes==true){
  if($web['guest_write']!=1){
    echo '<div style="position:relative;">
          <div id="ok">当前系统设置为不允许过客发表</div>
        </div>';
  }else{
    require('inc/function/send_mx.php');
    if($up_max=send_mx(abs($cookie[1]))){
      echo '<br /><br /><div style="text-align:center;padding:2px;line-height:normal;background-color:#FFCC00;">提示：当前您（'.(!isset($you)?'过客':'级别'.$youclass.'').'）每日最大发布限量为 '.$up_max.' 条</div>';
    }
  }
?>
